...
window.Pages.checkout = () => {
  ...
  if (cart.length === 0) {
    ...
    <div class="max-w-7xl mx-auto px-4 py-20 text-center page-enter">
      <div class="w-16 h-16 rounded-full bg-[#eefbfc] flex items-center justify-center text-[#007d83] mb-3 mx-auto">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-card p-6">
          ...
          <div class="space-y-3">
            ${[
              { id: 'standard', iconSvg: (window.Icons ? window.Icons.render('truck', 'w-5 h-5 text-[#007d83]') : ''), title: 'Standard Delivery', detail: '1-2 business days · Across Bangladesh', price: delivery === 0 ? 'FREE' : window.Utils.formatPrice(60) },
              { id: 'express', iconSvg: (window.Icons ? window.Icons.render('zap', 'w-5 h-5 text-[#007d83]') : ''), title: 'Express Same-Day Delivery', detail: 'Order before 12PM · Dhaka, Gazipur, Narayanganj', price: window.Utils.formatPrice(120) },
              ...
              <label class="payment-card ${id === selectedDelivery ? 'selected' : ''} flex items-center gap-4 p-3.5 rounded-lg border border-[#E5E7EB] cursor-pointer transition-all bg-white" id="delivery-${id}">
                <input type="radio" name="delivery" value="${id}" ${id === selectedDelivery ? 'checked' : ''} class="accent-[#007d83]" onchange="window.Pages.selectDelivery('${id}')" />
                <div class="w-9 h-9 rounded-lg bg-[#f4fdfe] flex items-center justify-center flex-shrink-0">${iconSvg}</div>
                <div class="flex-1">
        ...
        <div class="bg-white rounded-2xl shadow-card p-6">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ${[
              { id: 'cod', badgeText: 'COD', badgeBg: 'bg-[#007d83]', title: 'Cash on Delivery', detail: 'Pay cash upon delivery' },
              ...
              <label class="payment-card ${id === selectedPayment ? 'selected' : ''} flex items-center gap-3 p-3.5 rounded-lg border border-[#E5E7EB] cursor-pointer bg-white" id="payment-${id}">
                <input type="radio" name="payment" value="${id}" ${id === selectedPayment ? 'checked' : ''} class="accent-[#007d83]" onchange="window.Pages.selectPayment('${id}')" />
      ...
      <div class="lg:col-span-1">
        <div class="bg-white rounded-2xl shadow-card p-6 sticky-sidebar">
          ...
          <button
            ...
            id="place-order-btn"
            class="w-full bg-[#007d83] hover:bg-[#006065] active:scale-98 text-white font-bold py-3.5 rounded-lg transition-colors text-sm flex items-center justify-center gap-2 shadow-xs"
          >
